...
In JSON body templates, `{{ config.properties | split "," }}` converts a
comma-separated setting into an array, trimming whitespace and omitting empty
items. Body injection paths can address existing array elements, such as
`filters.0.value`.
...
For APIs that use a separate incremental endpoint, `incremental.request` can
override `path`, `method`, `query`, `body`, and `pagination`. Supplied components
replace their full-read counterparts; omitted components are preserved.
`disabled_when` names a boolean config setting that prevents incremental reads.

With `comparator: time`, `end_param` receives the extraction start time as a
fixed RFC3339 upper bound. Use `checkpoint_on_complete: true` when results are
not ordered by the watermark, including concurrent parent reads. This holds
checkpoint advancement until all selected reads complete; interrupted runs
replay from the previous watermark. Set `initial` for this mode.
