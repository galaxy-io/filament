...
Resend, Stripe, PostHog, Pipedrive, and Recharge sources all use this path.
...
- `cursor {response, request: query.X|body.X|header.X, more, null_terminates, continuation_query}`.
  `continuation_query` lists the only manifest query keys repeated on pages
  after the first, for APIs whose cursor already encodes the filters
