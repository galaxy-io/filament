...
Use `connection.rate_limit.responses` for provider-specific throttling signals:
...
Each rule requires `status`. Optional `header` requires a nonempty header;
`header_value` additionally requires an exact value. `body_path` is a GJSON path
whose string value must contain `body_contains` (case-insensitive). All conditions
in a rule must match; any matching rule identifies throttling. `backoff_seconds`
sets an exponential starting delay when `Retry-After` is absent. If multiple
rules match, the longest delay wins. HTTP 429 is always treated as throttling.
...
- HTTP 429 and manifest-matched throttling responses are retried within a
  ten-attempt budget and reported as `pressure.rate_limited`. Server-directed
  `Retry-After` delays are honored without a 60-second cap. Exhausted budgets
  honor the reset header and format configured in `rate_limit.dynamic`.
  Only the built-in fallback exponential backoff is capped at 60 seconds.
- 5xx responses are retried 3 times
- other 4xx responses are fatal

HTTP 204 produces an empty result without JSON decoding. For read endpoints
that return HTTP 202 while computing a result, set `response.poll_pending: true`.
The engine polls within the same ten-attempt budget and fails if data never
becomes ready; it does not emit the pending body as an empty snapshot. This flag
defaults to false and can be inherited from manifest response defaults.
