...
- HTTP 429 and identified HTTP 403 throttling responses are retried within a
  ten-attempt budget and reported as `pressure.rate_limited`. Server-directed
  `Retry-After` delays are honored without a 60-second cap. Exhausted GitHub
  budgets also honor `X-RateLimit-Reset`; recognized secondary limits wait at
  least a minute when no retry header is provided. Only fallback exponential
