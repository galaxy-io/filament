...
`conversation_types` list (default `public_channel`) that controls which
conversation types `conversations` enumerates. Select `private_channel`,
`mpim`, or `im` as well if the token has access.
