...
|---|---|---|
| `teams` | `Teams($first, $after, $updatedAfter)` | Incremental on `updatedAt` |
| `users` | `Users($first, $after, $updatedAfter)` | Incremental on `updatedAt` |
| `projects` | `Projects($first, $after, $updatedAfter)` | Incremental on `updatedAt` |
| `issues` | `Issues($first, $after, $updatedAfter)` | Incremental on `updatedAt`; denormalizes `team`, `state`, `assignee`, `creator`, and `project` as both a flat `_id` field and a nested `json` object |
...
All four resources support `ModeFull` and `ModeIncremental` (see
[HTTP](/pages/connectors/sources/http)). Incremental runs filter Linear's
GraphQL collections with `updatedAt >= <checkpoint>` and order results by
`updatedAt`. The queries include archived records, and the users query also
includes disabled accounts, so those lifecycle transitions remain visible.
Each resource maintains its own `updated_at` checkpoint.
