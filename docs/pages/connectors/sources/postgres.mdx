...
`pgoutput` plugin and bounded catch-up cycles, so it can run on a schedule:
each successful run stores its WAL LSN on the stable pipeline route and the next
run resumes there.
