...
New manifest-driven HTTP integrations start at **alpha** because their behavior
is derived from API documentation. Schema validation and mocked HTTP tests can
confirm that a manifest is internally consistent, but they do not prove it
against the live API.

An integration moves to **beta** after a successful live end-to-end run, and to
**stable** only after its supported resources and production behavior have been
fully verified. Maturity is set independently for each integration in the
connector registry and appears in the UI alongside its Source badge.
...
<CardGroup cols={2}>
  ...
  <Card title="PostHog" icon="https://cdn.getgalaxy.io/sources/source-icon-posthog-dark.svg" href="/pages/connectors/sources/posthog" />
...
- **`resources`** — one entry per extractable resource: its path, HTTP
  ...
  type), and how it paginates (`cursor`, `offset`, `page`, `link_header`, or
  `next_url`).
...
The HTTP source checkpoints the complete top-level pagination state, not just
a cursor string. A resumed run can therefore continue an offset, page number,
`Link` header, next URL, or cursor strategy without restarting the resource.
Older cursor-only checkpoints remain readable. Incremental resources persist
their watermark under `checkpoint_key` when one is declared, otherwise under
the cursor field name, so renaming a projected field does not have to discard
an intentionally stable checkpoint route.
