...
publish acknowledgments and its subject filter must capture the publish
subjects: the fixed subject itself, or `<prefix>.>` for prefix routing. Open
fails before any write when it does not. Note that a filter of `events.>`
matches `events.orders` but not the bare subject `events`. Each publish carries `Nats-Expected-Stream`, so a
