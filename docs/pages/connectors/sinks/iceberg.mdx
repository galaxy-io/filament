...
Iceberg uses the pipeline resource's sync mode. Replace, Append, Incremental,
and CDC compile to the corresponding internal Iceberg commit behavior; there
is no separate connector-level write-mode override.
