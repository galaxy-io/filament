...
} from "@/components/transform/actions";
import { parseTransformDefinition } from "@/components/transform/grammar/parse";
import { type PipelineTransformFieldsState, TransformStepKind } from "@/components/transform/types";
import { createTransformStep, placeTransformDraft } from "@/components/transform/utils";
...
function openStep(
  ...
  for (const [resource, steps] of state.stepsByResource) {
    ...
    return { ...state, draft: { resource, id: step.id, step: structuredClone(step) } };
  }
  return state;
}
