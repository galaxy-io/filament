...
import PipelineTransformFieldsArgument from "@/components/transform/PipelineTransformFieldsArgument";
...
const PipelineTransformFieldsCall = ({
  ...
  const setArgument = (position: number, expr: TransformExpr) => {
    const args = Array.from(
      ...
      (_, slot) => call.args[slot] ?? EMPTY_EXPR,
    );
    args[position] = expr;
    onChange({ ...call, args });
  };
          ...
          return (
            ...
            >
              <PipelineTransformFieldsArgument
                expr={call.args[position] ?? EMPTY_EXPR}
                spec={spec}
      ...
      {fn && isVariadic && (
        <PipelineTransformFieldsRow variant={PipelineTransformFieldsRowVariant.ARG} isAddRow>
          <Button
            ...
            onClick={() => onChange({ ...call, args: [...call.args, EMPTY_EXPR] })}
            isDisabled={editor.isDisabled}
          />
        </PipelineTransformFieldsRow>
      )}
