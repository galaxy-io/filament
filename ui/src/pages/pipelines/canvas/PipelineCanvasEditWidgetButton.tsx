...
const getIconVariant = (isActive: boolean, isPrimary: boolean, activeTheme: GalaxyTheme) => {
  if (isPrimary) {
    // Light: white icon on blue galaxy (inactive) and dark primaryAlt (active) backgrounds
    if (activeTheme === GalaxyTheme.LIGHT) {
      return IconVariant.PRIMARY_ALT;
    }
...
const StyledButton = withTheme(styled.button<
  PropsWithTheme<{ $isActive: boolean; $isPrimary: boolean }>
>`
  width: 32px;
  height: 32px;
  padding: 0;
...
const PipelineCanvasEditWidgetButton = ({
  ...
  return (
    <StyledButton $isActive={isActive} $isPrimary={isPrimary} onClick={onClick}>
      <Icon component={icon} variant={getIconVariant(isActive, isPrimary, activeTheme)} />
    </StyledButton>
  );
};
