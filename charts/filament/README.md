...
| existingSecret | string | `""` | Name of an existing Secret containing `PERSISTENCE_DSN`, `ENCRYPTION_KEY`, `NATS_URL`, and `AUTH_PAT` when auth is enabled. When set, the chart does not create its own Secret. |
