...
func runResumeScenario(t *testing.T, mode readMode, op gapOp) {
	...
	id, err := orch.Submit(ctx, filament.RunSubmission{Request: filament.RunRequest{
		Tenant:         "t1",
...
func countDoneShards(ctx context.Context, store *memory.Store, tenant filament.TenantID, id filament.RunID, resources []string) int {
	n := 0
