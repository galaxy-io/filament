	...
	for _, selection := range []string{"repository resources", "teams selected", "team members selected"} {
		t.Run(selection, func(t *testing.T) {
			...
			if selection != "repository resources" {
				if err == nil || !strings.Contains(err.Error(), "teams") || !strings.Contains(err.Error(), "HTTP 403") {
...
func TestGitHubManifestRateLimits(t *testing.T) {
	...
	for _, status := range []int{403, 429} {
		...
		if delay, limited := rateLimitDelay(resp, body, 0, m.Connection.RateLimit); !limited || delay != time.Minute {
			t.Fatalf("status=%d delay=%s limited=%v", status, delay, limited)
		}
		resp.Header.Set("Retry-After", "120")
		if delay, limited := rateLimitDelay(resp, body, 0, m.Connection.RateLimit); !limited || delay != 120*time.Second {
			t.Fatalf("explicit delay=%s limited=%v", delay, limited)
		}
	}
	resp := &http.Response{StatusCode: 403, Header: make(http.Header)}
	if _, limited := rateLimitDelay(resp, []byte(`{"message":"Resource not accessible by personal access token"}`), 0, m.Connection.RateLimit); limited {
		t.Fatal("permission denial classified as throttling")
	}
	resp.Header.Set("X-RateLimit-Remaining", "0")
	if _, limited := rateLimitDelay(resp, nil, 0, m.Connection.RateLimit); !limited {
