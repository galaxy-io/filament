...
func TestRateLimitDelay(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"reset budget", 403, map[string]string{"Budget-Remaining": "0", "Budget-Reset": strconv.FormatInt(time.Now().Add(5*time.Minute).Unix(), 10)}, nil, 0, 299 * time.Second, 300 * time.Second},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if !isRateLimited(resp, tc.body, spec) {
				t.Fatal("throttling was not recognized")
			}
			if got := rateLimitDelay(resp, tc.body, tc.attempt, spec); got < tc.minimum || got > tc.maximum {
...
func TestRateLimitMatchingIsManifestScoped(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := isRateLimited(resp, []byte(tc.body), spec); got != tc.want {
				t.Fatalf("configured=%v, want %v", got, tc.want)
			}
			if got := isRateLimited(resp, []byte(tc.body), manifest.RateLimit{}); got != (tc.status == 429) {
...
func TestRateLimitResetFormats(t *testing.T) {
	for _, tc := range []struct{ format, value string }{
		{"unix_seconds", strconv.FormatInt(time.Now().Add(5*time.Minute).Unix(), 10)},
		{"seconds_from_now", "300"},
		{"http_date", time.Now().Add(5 * time.Minute).UTC().Format(http.TimeFormat)},
	} {
		t.Run(tc.format, func(t *testing.T) {
			spec := throttleTestSpec(t)
			spec.Dynamic.ResetFormat = tc.format
			resp := &http.Response{StatusCode: 429, Header: make(http.Header)}
			resp.Header.Set("Budget-Remaining", "0")
			resp.Header.Set("Budget-Reset", tc.value)
			got := rateLimitDelay(resp, nil, 0, spec)
			if got < 299*time.Second || got > 300*time.Second {
				t.Fatalf("reset delay=%s", got)
			}
			if got := rateLimitDelay(resp, nil, 0, manifest.RateLimit{}); got != time.Second {
				t.Fatal("unconfigured reset headers changed delay")
			}
		})
	}
}
