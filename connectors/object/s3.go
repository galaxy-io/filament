...
func (s *Sink) Spec() ingestion.SinkSpec {
	return ingestion.SinkSpec{
		...
		Config: ingestion.ConfigSchema{Fields: []ingestion.ConfigField{
			{Name: "bucket", Type: ingestion.FieldString, Required: true, Help: "Destination S3 bucket."},
			{Name: "prefix", Type: ingestion.FieldString, Help: "Key prefix; objects land at <prefix>/<run>/<resource>.ndjson."},
			{Name: "region", Type: ingestion.FieldString, Help: "AWS region; defaults to the SDK's resolved region."},
			{Name: "endpoint", Type: ingestion.FieldString, Help: "Custom S3 endpoint (e.g. MinIO); defaults to AWS."},
			{Name: "access_key_id", Type: ingestion.FieldSecret, Help: "Static access key; omit to use the SDK credential chain."},
			{Name: "secret_access_key", Type: ingestion.FieldSecret, Help: "Static secret key; omit to use the SDK credential chain."},
			{Name: "part_size_mib", Type: ingestion.FieldInt, Help: "Multipart part size in MiB; min 5, default 16."},
			{Name: "upload_concurrency", Type: ingestion.FieldInt, Help: "Max in-flight part uploads across all resources; default 8."},
		}},
