...
// single source of truth for every metric this store supports. Times are
// unix milliseconds, so duration is a plain difference; avg() ignores NULLs,
// and ended_at is only set on terminal runs, so this is naturally "mean
// duration over terminal runs" with no extra filtering. The usage columns
// are NOT NULL DEFAULT 0 and zero means "no usage reported" (runs outside a
// cgroup never heartbeat), so nullif routes those rows through the same
// avg()-ignores-NULLs path.
//
