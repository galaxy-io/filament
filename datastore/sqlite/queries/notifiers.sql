...
UPDATE notifier SET name = @name, is_enabled = @is_enabled,
  ...
  updated_at = @updated_at, updated_by_user_id = sqlc.narg(updated_by_user_id)
WHERE tenant_id = @tenant_id AND pipeline_id = @pipeline_id AND id = @notifier_id AND is_deleted = 0
RETURNING *;

-- name: DeleteNotifier :one
UPDATE notifier SET is_deleted = 1, deleted_at = @deleted_at, updated_at = @updated_at
WHERE tenant_id = @tenant_id AND pipeline_id = @pipeline_id AND id = @notifier_id AND is_deleted = 0
RETURNING *;
