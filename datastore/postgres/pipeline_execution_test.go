...
func TestPipelineExecutionPersistence(t *testing.T) {
	...
	p, err := store.CreatePipelineWithSchedule(ctx, &ingestionv1.Pipeline{Id: pipelineOne, TenantId: tenantA, Name: "events", Execution: ingestionv1.ExecutionMode_EXECUTION_MODE_CONTINUOUS}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if p.Execution != ingestionv1.ExecutionMode_EXECUTION_MODE_CONTINUOUS {
	...
	if err != nil || len(list) != 1 || list[0].Execution != ingestionv1.ExecutionMode_EXECUTION_MODE_CONTINUOUS {
		t.Fatalf("list: %v %v", list, err)
	}
	p, err = store.UpdatePipeline(ctx, &ingestionv1.Pipeline{Id: pipelineOne, TenantId: tenantA, Name: "bounded", Execution: ingestionv1.ExecutionMode_EXECUTION_MODE_BOUNDED})
