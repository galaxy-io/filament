...
UPDATE notifier SET name = @name, is_enabled = @is_enabled,
  ...
  updated_at = now(), updated_by_user_id = sqlc.narg(updated_by_user_id)
WHERE tenant_id = @tenant_id AND pipeline_id = @pipeline_id AND id = @notifier_id AND NOT is_deleted
RETURNING *;
