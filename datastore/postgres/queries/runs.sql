...
VALUES (@run_id, @tenant_id, nullif(@schedule_id::text, ''), @status, @request, @records, @bytes, @started_at, @finished_at, nullif(@error::text, ''), @cpu_seconds, @memory_peak_bytes, now())
