...
func TestSaveRunRejectsOverlappingCheckpointRoute(t *testing.T) {
	...
	if err := store.SaveRun(ctx, filament.RunState{Run: "run-a", Tenant: "t-1", Status: filament.RunRunning, Request: request}); err != nil {
		t.Fatal(err)
	}
	if err := store.SaveRun(ctx, filament.RunState{Run: "run-b", Tenant: "t-1", Status: filament.RunRequested, Request: request}); err == nil {
		t.Fatal("expected overlapping run rejection")
	}
	first, err := store.LoadRun(ctx, "t-1", "run-a")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPartialRunDoesNotBlockNextScheduledAttempt(t *testing.T) {
	...
	store := New()
	request := filament.RunRequest{PipelineID: "pipe", PipelineVersionID: "version-1", CheckpointRoute: "route/source/sink/upsert"}
	if err := store.SaveRun(ctx, filament.RunState{Run: "run-a", Tenant: "t-1", Status: filament.RunPartial, Request: request}); err != nil {
		t.Fatal(err)
	}
	if err := store.SaveRun(ctx, filament.RunState{Run: "run-b", Tenant: "t-1", Status: filament.RunRequested, Request: request}); err != nil {
