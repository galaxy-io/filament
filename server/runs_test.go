...
func TestLoadRunSnapshotOnlySuppressesNotFound(t *testing.T) {
	...
	server := &Server{store: loadRunErrorStore{DataStore: sqlitestore.NewMemory(), err: storeErr}}
	...
	server.store = loadRunErrorStore{DataStore: sqlitestore.NewMemory(), err: filament.ErrNotFound}
