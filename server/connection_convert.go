...
func connectionFromProto(c *ingestionv1.Connection) filament.Connection {
	...
	return filament.Connection{ID: c.GetId(), Tenant: c.GetTenant(), Kind: connectionKindFromProto(c.GetKind()), Name: c.GetName(), Connector: c.GetConnector(), Config: structMap(c.GetConfig()), SecretRefs: cloneStrings(c.GetSecretRefs()), Version: c.GetVersion()}
}
