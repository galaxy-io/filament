...
func connectionFromProto(c *ingestionv1.Connection) filament.Connection {
	...
	return filament.Connection{ID: c.GetId(), Tenant: c.GetTenantId(), Kind: connectionKindFromProto(c.GetKind()), Name: c.GetName(), Connector: c.GetConnector(), Config: structMap(c.GetConfig()), SecretRefs: cloneStrings(c.GetSecretRefs()), Version: c.GetVersion(), DeletedAt: c.GetDeletedAt()}
}

func connectionToProto(c filament.Connection) *ingestionv1.Connection {
	cfg, _ := structpb.NewStruct(c.Config)
	return &ingestionv1.Connection{Id: c.ID, TenantId: c.Tenant, Kind: connectionKindToProto(c.Kind), Name: c.Name, Connector: c.Connector, Config: cfg, SecretRefs: cloneStrings(c.SecretRefs), Version: c.Version, DeletedAt: c.DeletedAt}
}
